                        ...
                        bool* pfMissingInputs)
{
    ...
    string strNonStd;
    if (!(user_options.count("testnet") and user_options["testnet"].as<bool>()) && !tx.IsStandard(strNonStd))
    ...
    if (fCheckInputs)
    {
        ...
        if (!tx.AreInputsStandard(view) && !(user_options.count("testnet") and user_options["testnet"].as<bool>()))
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            if (dFreeCount >= user_options["limitfreerelay"].as<unsigned int>()*10*1000)
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    if (user_options.count("testnet") and user_options["testnet"].as<bool>() && nTime > nTargetSpacing*2)
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    std::string strCmd = user_options["blocknotify"].as<std::string>();
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        if (nVersion < 2)
        {
	  if ((!(user_options.count("testnet") and user_options["testnet"].as<bool>()) && CBlockIndex::IsSuperMajority(2, pindexPrev, 950, 1000)) ||
                (user_options.count("testnet") and user_options["testnet"].as<bool>() && CBlockIndex::IsSuperMajority(2, pindexPrev, 75, 100)))
            {
	  ...
	  if ((!(user_options.count("testnet") and user_options["testnet"].as<bool>()) && CBlockIndex::IsSuperMajority(2, pindexPrev, 750, 1000)) ||
                (user_options.count("testnet") and user_options["testnet"].as<bool>() && CBlockIndex::IsSuperMajority(2, pindexPrev, 51, 100)))
            {
...
{
  uint64 nFreeBytesAvailable = filesystem::space(user_options["datadir"].as<fs::path>()).available;
...
FILE* OpenDiskFile(const CDiskBlockPos &pos, const char *prefix, bool fReadOnly)
{
    ...
    boost::filesystem::path path = 
      user_options["datadir"].as<fs::path>() 
      / "blocks" 
      / strprintf("%s%05u.dat", prefix, pos.nFile);
...
bool InitBlockIndex() {
    ...
    fTxIndex = user_options["txindex"].as<bool>();
...
string GetWarnings(string strFor)
{
    ...
    if (user_options["testsafemode"].as<bool>()) {
        strRPC = "test";
    }
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        if (pfrom->nSendSize >= user_options["maxsendbuffer"].as<unsigned int>())
            break;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
  ...
  if (user_options.count("dropmessagestest") 
      and GetRand(user_options["dropmessagestest"].as<unsigned int>()) == 0) {
  ...
  if (strCommand == "version") {
	...
	if (user_options.count("listen") and user_options["listen"].as<bool>() and !IsInitialBlockDownload()) {
  ...
  else if (strCommand == "getdata")
    {
      ...
      if ((user_options.count("debugnet") and user_options["debugnet"].as<bool>()) or (vInv.size() != 1)) {
	printf("received getdata (%"PRIszu" invsz)\n", vInv.size());
      }
      
      if ((user_options.count("debugnet") and user_options["debugnet"].as<bool>() && vInv.size() > 0) || (vInv.size() == 1))
  ...
  } else if (!(user_options.count("bloomfilters") and user_options["bloomfilters"].as<bool>())
	     and (strCommand == "filterload" 
		  or strCommand == "filteradd" 
		  or strCommand == "filterclear")) {
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
  ...
  if (lockMain) {
	...
	if (!AlreadyHave(inv))
	  {
	    if (user_options.count("debugnet") and user_options["debugnet"].as<bool>())
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
  ...
  unsigned int nBlockMaxSize = user_options["blockmaxsize"].as<unsigned int>();
  // Limit to betweeen 1K and MAX_BLOCK_SIZE-1K for sanity:
  nBlockMaxSize = std::max((unsigned int)1000, std::min((unsigned int)(MAX_BLOCK_SIZE-1000), nBlockMaxSize));
  
  // How much of the block should be dedicated to high-priority transactions,
  // included regardless of the fees they pay
  unsigned int nBlockPrioritySize = user_options["blockprioritysize"].as<unsigned int>();
  nBlockPrioritySize = std::min(nBlockMaxSize, nBlockPrioritySize);
  
  // Minimum block size you want to create; block will be filled with free transactions
  // until there are no more or the block reaches this size:
  unsigned int nBlockMinSize = user_options["blockminsize"].as<unsigned int>();
  ...
  int64 nFees = 0;
  {
    ...
    bool fPrintPriority = user_options["printpriority"].as<bool>();
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
  ...
  int nThreads = user_options["genproclimit"].as<int>();
  if (nThreads < 0) {
