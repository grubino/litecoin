...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    value tests = read_json("tx_valid.json");

    BOOST_FOREACH(value& test, boost::get<array_t>(tests.get_ast())) {
      string strTest = write_string(test, false);
      if (test[0].type() == array_type) {
	if (test.length() != 3 || test[1].type() != string_type || test[2].type() != bool_type) {
	...
	value inputs = test[0];
	bool fValid = true;
	BOOST_FOREACH(value& input, boost::get<array_t>(inputs.get_ast())) {
	  ...
	  mapprevOutScriptPubKeys[COutPoint(uint256(input[0].get_as<std::string>()), input[1].get_as<int>())] = ParseScript(input[2].get_as<std::string>());
	}
	if (!fValid) {
	  BOOST_ERROR("Bad test: " << strTest);
	  continue;
	...
	string transaction = test[1].get_as<std::string>();
	...
	for (unsigned int i = 0; i < tx.vin.size(); i++) {
	  ...
	  BOOST_CHECK_MESSAGE(VerifyScript(tx.vin[i].scriptSig, mapprevOutScriptPubKeys[tx.vin[i].prevout], tx, i, test[2].get_as<bool>() ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE, 0), strTest);
	}
      }
    }
}
...
BOOST_AUTO_TEST_CASE(tx_invalid) {
  ...
  // or [[[prevout hash, prevout index, prevout scriptPubKey], [input 2], ...],"], serializedTransaction, enforceP2SH
  // ... where all scripts are stringified scripts.
  value tests = read_json("tx_invalid.json");
  ...
  BOOST_FOREACH(value& test, boost::get<array_t>(tests.get_ast())) {
      ...
      for (unsigned int i = 0; i < tx.vin.size() && fValid; i++)
	{
	  ...
	  fValid = VerifyScript(tx.vin[i].scriptSig
				...
				, tx
				, i
				, test[2].get_as<bool>() ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE
				, 0);
	}
