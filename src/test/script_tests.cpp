...
BOOST_AUTO_TEST_CASE(script_valid)
{
  ...
  // scripts.
  value tests = read_json("script_valid.json");
  ...
  BOOST_FOREACH(value& test, boost::get<array_t>(tests.get_ast())) {
    ...
    string scriptSigString = test[0].get_as<std::string>();
    CScript scriptSig = ParseScript(scriptSigString);
    string scriptPubKeyString = test[1].get_as<std::string>();
...
BOOST_AUTO_TEST_CASE(script_invalid)
{
  ...
  value tests = read_json("script_invalid.json");
  
  BOOST_FOREACH(value& tv, boost::get<array_t>(tests.get_ast())) {
    value test = tv;
    string strTest = write_string(tv, false);
    if (test.length() < 2) // Allow size > 2; extra stuff ignored (useful for comments)
      {
	BOOST_ERROR("Bad test: " << strTest);
	continue;
      }
